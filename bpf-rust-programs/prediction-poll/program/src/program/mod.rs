...
use prediction_poll_data::{ClockData, CollectionData, PollData, InitPollData, TallyData};
...
pub fn process_instruction(
    ...
    match command {
        ...
        Command::SubmitVote => submit_vote(keyed_accounts),
        Command::SubmitClaim => submit_claim(keyed_accounts),
    }
}
...
fn init_poll(keyed_accounts: &mut [SolKeyedAccount], init_data: &[u8]) -> ProgramResult<()> {
    ...
    let poll_data = PollData::init(init_poll, tally_a_account.key, tally_b_account.key, clock.slot);
