...
impl<'a> PollData<'a> {
    pub fn length(&self) -> usize {
        (8 + 4 + self.header_len) as usize + self.option_a.length() + self.option_b.length()
    }
    ...
    // TODO error checking
    pub fn init(init: InitPollData<'a>, tally_a_key: &'a SolPubkey, tally_b_key: &'a SolPubkey, slot: u64) -> Self {
        Self {
